...
const SidebarActions = () => {
  ...
  return (
    <div className="w-full h-8 flex justify-end items-center gap-3">
      <ConverSationModal setSelectedUser={setSelectedUser} />
      <FilterOption setFilter={setFilter} currentFilter={currentFilter} />
    </div>
  );
};
  ...
  return (
    <>
      ...
      <button
        ...
        className="transition-colors duration-200 hover:bg-accent-content/40 hover:text-accent cursor-pointer p-2 rounded-full"
      >
        <Edit />
      </button>
...
function FilterOption({
  ...
  return (
    <div className="dropdown dropdown-end">
      ...
      >
        <button
          onClick={() => setFilter(FilterType.Online)}
          className={`${
            currentFilter === FilterType.Online && "bg-accent/40"
          } cursor-pointer hover:bg-accent-content hover:text-accent p-2 text-base rounded-md inline-flex items-center gap-2`}
        >
